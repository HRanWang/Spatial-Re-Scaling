...
#===================================================================================
            # loss_layer = (loss_layer1+loss_layer2+loss_layer4+loss_layer3)/4
            loss = loss_layer4
            # loss = 0.2*loss_layer+0.8*loss_
            # loss = (loss0+loss1+loss2+loss3+loss4+loss5+loss6+loss7+loss8+loss9+loss10+loss11)/12
            losses.update(loss.data[0], targets.size(0))
            precisions.update(prec1, targets.size(0))

            optimizer.zero_grad()
            torch.autograd.backward([loss_layer4],
                                    [torch.ones(1).cuda()
                                     ])
            optimizer.step()
...
class Trainer(BaseTrainer):
    ...
    def _forward(self, inputs, targets):
        ...
        if isinstance(self.criterion, torch.nn.CrossEntropyLoss):
            ...
            # loss8 = self.criterion(outputs[1][10],targets)
            # loss9 = self.criterion(outputs[1][11],targets)
            # loss10 = self.criterion(outputs[1][12],targets)
            # loss11 = self.criterion(outputs[1][13], targets)
            # loss12 = self.criterion(outputs[1][14], targets)
